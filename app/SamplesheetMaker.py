...
debug = False
if debug == False:
    # get path to index directory from script location, regardless of who's Dropbox is being used
    scriptPath = Path(sys.argv[0])
    inputFile = Path(sys.argv[1])
    scriptPath = Path(os.path.abspath(scriptPath))
    inputFile = Path(os.path.abspath(inputFile))

    if len(sys.argv) != 2:
        print("Usage - enter the following into terminal: python SamplesheetMaker.py inputFile.csv")
        print('Generated samplesheet is output to the same directory as inputFile.csv')
        exit()

if debug == True:
    #debug paths
    scriptPath = Path(r"C:\Users\markg\Partners HealthCare Dropbox\Mark Godek\Shalek Lab\Protocols-Experiments\Comp Scripts\SampleSheetMaking\SmartSeq\SamplesheetMaker.py")
    inputFile = r"C:\Users\markg\Partners HealthCare Dropbox\Mark Godek\Shalek Lab\Protocols-Experiments\Comp Scripts\SampleSheetMaking\SmartSeq\Run_1390_Input.csv"#SmartSeqInputTemplate.csv"
...
def indexReader(indexID,indices):
    indexFile = str(scriptDir.parent) + '\\' + indexID + '.csv'
    try:
...
with open(inputFile, 'r') as runInfo:
    ...
    header.append(['Sample_ID', 'Sample_Name', 'Sample_Plate', 'Sample_Well', 'I7_Index_ID', 'index', 'I5_Index_ID', 'index2', 'Sample_Project'])
    ...
    with open(outFileName,'w',newline='') as sampleSheet:
        ...
        for i in range(len(i7Indices)):
            ...
            outstring = sampleID, sampleNames[i], plateIDForRows[i], wellsForRows[i], i7IDForRow[i], i7Indices[i],\
